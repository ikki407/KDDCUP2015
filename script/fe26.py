...
aaa = X['username'].drop_duplicates().values
#保存するdf
df = pd.DataFrame(np.zeros(len(X)*2).reshape(2,len(X)).T,columns=['enrollment_id','into+1-max'])
index = 0
for i in aaa:#各usernameごとにfor
    ee = X.iloc[(X['username']==i).values,:]
    if len(ee)>1:
        for j in range(0,len(ee)):
            ...
            aa = range(0,len(ee))
            aa.remove(j)
            count_prev = 0#10日間に他のコースで参加した回数(初期化)
            for k in aa:
                ee3 = ee.iloc[k,:]#比較するデータ
                
                if (ee2['max'] <= ee3['max_time'] < ee2['to+1a']) :
                    count_prev += 1
                    
            df.iloc[index,:] = [ee2['enrollment_id'],count_prev]
            index += 1
    else:
...
df.to_csv('/Users/IkkiTanaka/Documents/KDDCup/fe_train/dffe26.csv',index=False)

df_ = pd.DataFrame(df.iloc[df['enrollment_id'].isin(dtrain['enrollment_id'].values).values,:][['into+1-max']].values,columns=['into+1-max'])
dtrain = pd.concat([dtrain,df_],axis=1)

df_ = pd.DataFrame(df.iloc[df['enrollment_id'].isin(dtest['enrollment_id'].values).values,:][['into+1-max']].values,columns=['into+1-max'])
dtest = pd.concat([dtest,df_],axis=1)

dtrain['in10days_enr+in20days_enr+into+1-max'] = dtrain['in10days_enr'] + dtrain['in20days_enr'] + dtrain['into+1-max']
dtest['in10days_enr+in20days_enr+into+1-max'] = dtest['in10days_enr'] + dtest['in20days_enr'] + dtest['into+1-max']
