...
del dtrain['username'],dtrain['course_id'],dtrain['enrollment_id'],dtrain['username_x']
del dtest['username'], dtest['course_id'],dtest['enrollment_id'],dtest['username_x']
...
for i in xrange(0,num_model):
    ...
    param = {"objective" : "binary:logistic",#"binary:logistic",
              ...
              "num_class" :  1,
              "nthread" :  8,
              "bst:max_depth" :  random.randint(8,9),
    ...
    num_round = int( round(random.uniform(1100, 1150),0))
...
pred3 = pd.read_csv('/Users/IkkiTanaka/Documents/KDDCup/pred/xgb/pred_all42_0.898267_1730_6.csv',header=None)

final_pred = (pred1[1]+pred2[1]+pred3[1])/3.0
final_pred = pd.concat([pred1[0],final_pred],axis=1)
final_pred.to_csv('/Users/IkkiTanaka/Documents/KDDCup/pred/xgb/PRED_ALL4.csv',header=None,index=False)
...
for i in xrange(0,num_model):
    ...
    #if random.randint(0,1):
    #    param['scale_pos_weight'] = scale_pos_weight
    num_round = int( round(random.uniform(1400, 1500),0))
    #k = int(round(1+np.log2(dtrain_all_sf.num_row())))
    print param
    print num_round
    #print k
