...
dtrain['sum_count_cg'] = dtrain[[ u'chapter_cg', u'problem_cg', u'sequential_cg', u'video_cg']].sum(1)
#sum_countで割って, percentageを追加する
for col in [ u'chapter_cg', u'problem_cg', u'sequential_cg', u'video_cg']:
    dtrain[col+'_pct'] = dtrain[col]/dtrain[u'sum_count_cg']

dtest['sum_count_cg'] = dtest[[ u'chapter_cg',  u'problem_cg', u'sequential_cg', u'video_cg']].sum(1)#u'combinedopenended_cg',
#sum_countで割って, percentageを追加する
for col in [ u'chapter_cg',  u'problem_cg', u'sequential_cg', u'video_cg']:
...
#sum_countで割って, percentageを追加する
for col in label_to1:
    dtrain[col+'_pct'] = dtrain[col]/dtrain[u'sum_count']

for col in label_to1:
    dtest[col+'_pct'] = dtest[col]/dtest[u'sum_count']

dtrain['2013_10_pct'] = dtrain['10_m_pct']
dtrain['2013_11_pct'] = dtrain['11_m_pct']
dtrain['2013_12_pct'] = dtrain['12_m_pct']
dtrain['2014_01_pct'] = dtrain['01_m_pct']
dtrain['2014_02_pct'] = dtrain['02_m_pct']
dtrain['2014_05_pct'] = dtrain['05_m_pct']
dtrain['2014_06_pct'] = dtrain['06_m_pct']
dtrain['2014_07_pct'] = dtrain['07_m_pct']
dtrain['2014_08_pct'] = dtrain['08_m_pct']

del dtrain['2013_pct'],dtrain['2014_pct'],dtrain['10_m_pct'],dtrain['11_m_pct'],dtrain['12_m_pct'],dtrain['01_m_pct'],dtrain['02_m_pct'],dtrain['05_m_pct'],dtrain['06_m_pct'],dtrain['07_m_pct'],dtrain['08_m_pct']

dtest['2013_10_pct'] = dtest['10_m_pct']
dtest['2013_11_pct'] = dtest['11_m_pct']
dtest['2013_12_pct'] = dtest['12_m_pct']
dtest['2014_01_pct'] = dtest['01_m_pct']
dtest['2014_02_pct'] = dtest['02_m_pct']
dtest['2014_05_pct'] = dtest['05_m_pct']
dtest['2014_06_pct'] = dtest['06_m_pct']
dtest['2014_07_pct'] = dtest['07_m_pct']
dtest['2014_08_pct'] = dtest['08_m_pct']

del dtest['2013_pct'],dtest['2014_pct'],dtest['10_m_pct'],dtest['11_m_pct'],dtest['12_m_pct'],dtest['01_m_pct'],dtest['02_m_pct'],dtest['05_m_pct'],dtest['06_m_pct'],dtest['07_m_pct'],dtest['08_m_pct']
...
for i in xrange(0,num_model):
    ...
    param = {"objective" : "binary:logistic",#"binary:logistic",
              ...
              "num_class" :  1,
              "nthread" :  8,
              "bst:max_depth" :  random.randint(5,7),
              ...
              "bst:subsample" :  round(random.uniform(.5, 1.),2),#row_subsample
              "colsample_bytree" :  round(random.uniform(.5, 1.),2),#column_subsample
              "silent": 1,
    ...
    if cc > 0.89045:
        ...
        pred_name.append('/Users/IkkiTanaka/Documents/KDDCup/pred/xgb/pred_all3%i_%f_%i_%i.csv' % (i,cc,num_round_, param["bst:max_depth"]))
        preds.to_csv('/Users/IkkiTanaka/Documents/KDDCup/pred/xgb/pred_all3%i_%f_%i_%i.csv' % (i,cc,num_round_, param["bst:max_depth"]) ,header=None,index=False)
...
ccc = bbb[['course_id','ratio_of_drop']]
ccc['drop'] = aaa['drop']
dtrain.merge(ccc,on='course_id',how='left')

#0.8869645952505807
{'bst:eta': 0.0738298477076178,
 'bst:gamma': 4.7,
 'bst:max_depth': 3,
 ...
 'silent': 1}
num_round_ = 1141
pred_all0_0.886225_1140_3

#num_course_countを間違えた時のscore
#0.8916107431796965
{'bst:subsample': 0.6, 'eval_metric': 'auc', 'base_score': 0.53, 'num_class': 1, 'bst:eta': 0.04389734947748931, 'colsample_bytree': 0.56, 'silent': 1, 'bst:gamma': 6.1, 'nthread': 8, 'bst:max_depth': 4, 'objective': 'binary:logistic', 'bst:min_child_weight': 1}
529
pred_all20_0.890391_455_4

#0.8919185461399682
{'bst:subsample': 1.0, 'eval_metric': 'auc', 'base_score': 0.54, 'num_class': 1, 'bst:eta': 0.019612045652688377, 'colsample_bytree': 0.21, 'silent': 1, 'bst:gamma': 0.6, 'nthread': 8, 'bst:max_depth': 7, 'objective': 'binary:logistic', 'bst:min_child_weight': 2}
486
pred_all21_0.890779_543_7
...
#f.head(30)[1]
using_label = fi.head(70)[0].values
...
Object.iloc[(Object['category']=='sequential').values,:]['course_id'].value_counts()
...
train = pd.read_csv('/Users/IkkiTanaka/Documents/KDDCup/train/log_train.csv')
test = pd.read_csv('/Users/IkkiTanaka/Documents/KDDCup/test/log_test.csv')
train2 = train.drop_duplicates(cols=[ u'username', u'course_id', u'source', u'event', u'object'])
f_time = lambda x: date(int(x[:4]), int(x[5:7]), int(x[8:10])).toordinal() - 735000 if x != 'null' else np.nan
rrr = train2.reset_index().merge(Object, left_on='object',right_on='module_id',how='left').sort('index').drop('index',axis=1)[['object','category','start']]
rrr.iloc[(rrr['category']=='chapter').values,:]
rrr['start'] = rrr['start'].fillna('null').apply(f_time)
rrr.dropna()
test = test.drop_duplicates()

enr_train = pd.read_csv('/Users/IkkiTanaka/Documents/KDDCup/train/enrollment_train.csv')
enr_test = pd.read_csv('/Users/IkkiTanaka/Documents/KDDCup/test/enrollment_test.csv')
label2 = pd.read_csv('/Users/IkkiTanaka/Documents/KDDCup/train/truth_train.csv',header=None)
label2.columns = ['enrollment_id','drop']
enr_train = enr_train.reset_index().merge(label2, how='inner',on='enrollment_id').sort('index').drop('index',axis=1)#ここで1つデータが落ちている

yyy = enr_train.groupby('username')['drop'].sum()
yyy = pd.DataFrame(yyy)
yyy['username'] = yyy.index
...
dtest_leaf['sum_drop'] = dtest_leaf.reset_index().merge(yyy,on='username',how='left').sort('index').drop('index',axis=1)['drop'].values


pred1 = pd.read_csv('/Users/IkkiTanaka/Documents/KDDCup/pred/xgb/pred_all37_0.890907_466_8.csv',header=None)
pred2 = pd.read_csv('/Users/IkkiTanaka/Documents/KDDCup/pred/xgb/pred_all32_0.890928_398_6.csv',header=None)
pred3 = pd.read_csv('/Users/IkkiTanaka/Documents/KDDCup/pred/xgb/pred_all36_0.890827_812_6.csv',header=None)
pred4 = pd.read_csv('/Users/IkkiTanaka/Documents/KDDCup/pred/xgb/pred_all31_0.890795_711_7.csv',header=None)


final_pred = (pred1[1]+pred2[1]+pred3[1])/3.0
final_pred = pd.concat([pred1[0],final_pred],axis=1)
final_pred.to_csv('/Users/IkkiTanaka/Documents/KDDCup/pred/xgb/Pred52.csv',header=None,index=False)




new_X = np.zeros(len(X)*4).reshape(len(X),4)
index = np.zeros(len(X))
count = 0
r = 0
for i in username[:100]:
    username_data = X.loc[(X['username']==i).values,:]
    if len(username_data['drop'].values) > 1:#参加数が1回の人は欠損値とする
        a = username_data['drop']
        past_enr = range(1,len(a)+1)
        past_drop = a.cumsum()
        username_data['past_drop'] = past_drop.fillna(method='ffill').fillna(method='bfill')
    else:
        a = np.nan
        past_enr = 1
        past_drop = np.nan
    ...
    #new_X = pd.concat([new_X, username_data[['past_enr','past_drop','past_nondrop','past_nondrop_rate']]])
    new_X[r:(len(username_data)+r),:] = username_data[['past_enr','past_drop','past_nondrop','past_nondrop_rate']].values
    index[r:(len(username_data)+r)] = username_data.index
    r += len(username_data)
    print count
    count += 1
    if count % 1000 == 0:
