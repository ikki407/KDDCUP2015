...
#loginしてから初めてvideoを見るまでの時間
train = pd.read_csv('/Users/IkkiTanaka/Documents/KDDCup/train/log_train_depth.csv')
test = pd.read_csv('/Users/IkkiTanaka/Documents/KDDCup/test/log_test_depth.csv')
train = train.sort('enrollment_id')
test = test.sort('enrollment_id')
#problemのみを抜き出す
train = train.iloc[(train['event']=='video').values,:]
test = test.iloc[(test['event']=='video').values,:]
f_time = lambda x: date(int(x[:4]), int(x[5:7]), int(x[8:10])).toordinal() - 735000
train['time'] = train['time'].apply(f_time)
test['time'] = test['time'].apply(f_time)

#train
aa = train.groupby('enrollment_id').min()['time']
aa = pd.DataFrame(aa)
aa['enrollment_id'] = aa.index
aa.columns = ['min_time_video','enrollment_id']
dtrain = dtrain.reset_index().merge(aa,on='enrollment_id',how='left').sort('index').drop('index',axis=1)

aa = train.groupby('enrollment_id').max()['time']
aa = pd.DataFrame(aa)
aa['enrollment_id'] = aa.index
aa.columns = ['max_time_video','enrollment_id']
dtrain = dtrain.reset_index().merge(aa,on='enrollment_id',how='left').sort('index').drop('index',axis=1)

aa = train.groupby('enrollment_id').mean()['time']
aa = pd.DataFrame(aa)
aa['enrollment_id'] = aa.index
aa.columns = ['mean_time_video','enrollment_id']
dtrain = dtrain.reset_index().merge(aa,on='enrollment_id',how='left').sort('index').drop('index',axis=1)

aa = train.groupby('enrollment_id').std()['time']
aa = pd.DataFrame(aa)
aa['enrollment_id'] = aa.index
aa.columns = ['std_time_video','enrollment_id']
dtrain = dtrain.reset_index().merge(aa,on='enrollment_id',how='left').sort('index').drop('index',axis=1)

dtrain.index = range(0,len(dtrain))
#loginしてからvideoを見るまでの時間
dtrain['min_time_video-min_time'] = dtrain['min_time_video'] - dtrain['min_time']
#最後にvideoをみてからlogoutするまでの時間
dtrain['max_time-max_time_video'] = dtrain['max_time'] - dtrain['max_time_video']
#videoを最後に見た時間と最初に見た時間との差
dtrain['max_time_video-min_time_video'] = dtrain['max_time_video'] - dtrain['min_time_video']

#test
aa = test.groupby('enrollment_id').min()['time']
aa = pd.DataFrame(aa)
aa['enrollment_id'] = aa.index
aa.columns = ['min_time_video','enrollment_id']
dtest = dtest.reset_index().merge(aa,on='enrollment_id',how='left').sort('index').drop('index',axis=1)

aa = test.groupby('enrollment_id').max()['time']
aa = pd.DataFrame(aa)
aa['enrollment_id'] = aa.index
aa.columns = ['max_time_video','enrollment_id']
dtest = dtest.reset_index().merge(aa,on='enrollment_id',how='left').sort('index').drop('index',axis=1)

aa = test.groupby('enrollment_id').mean()['time']
aa = pd.DataFrame(aa)
aa['enrollment_id'] = aa.index
aa.columns = ['mean_time_video','enrollment_id']
dtest = dtest.reset_index().merge(aa,on='enrollment_id',how='left').sort('index').drop('index',axis=1)

aa = test.groupby('enrollment_id').std()['time']
aa = pd.DataFrame(aa)
aa['enrollment_id'] = aa.index
aa.columns = ['std_time_video','enrollment_id']
dtest = dtest.reset_index().merge(aa,on='enrollment_id',how='left').sort('index').drop('index',axis=1)
...
dtest.index = range(0,len(dtest))
#loginしてからvideoを見るまでの時間
dtest['min_time_video-min_time'] = dtest['min_time_video'] - dtest['min_time']
#最後にvideoをみてからlogoutするまでの時間
dtest['max_time-max_time_video'] = dtest['max_time'] - dtest['max_time_video']
#videoを最後に見た時間と最初に見た時間との差
dtest['max_time_video-min_time_video'] = dtest['max_time_video'] - dtest['min_time_video']
...
#loginしてから初めてproblemを見るまでの時間
train = pd.read_csv('/Users/IkkiTanaka/Documents/KDDCup/train/log_train_depth.csv')
test = pd.read_csv('/Users/IkkiTanaka/Documents/KDDCup/test/log_test_depth.csv')
train = train.sort('enrollment_id')
test = test.sort('enrollment_id')
#problemのみを抜き出す
train = train.iloc[(train['event']=='problem').values,:]
test = test.iloc[(test['event']=='problem').values,:]
f_time = lambda x: date(int(x[:4]), int(x[5:7]), int(x[8:10])).toordinal() - 735000
train['time'] = train['time'].apply(f_time)
test['time'] = test['time'].apply(f_time)
...
aa = pd.DataFrame(aa)
aa['enrollment_id'] = aa.index
aa.columns = ['min_time_problem','enrollment_id']
dtrain = dtrain.reset_index().merge(aa,on='enrollment_id',how='left').sort('index').drop('index',axis=1)

aa = train.groupby('enrollment_id').max()['time']
aa = pd.DataFrame(aa)
aa['enrollment_id'] = aa.index
aa.columns = ['max_time_problem','enrollment_id']
dtrain = dtrain.reset_index().merge(aa,on='enrollment_id',how='left').sort('index').drop('index',axis=1)

aa = train.groupby('enrollment_id').mean()['time']
aa = pd.DataFrame(aa)
aa['enrollment_id'] = aa.index
aa.columns = ['mean_time_problem','enrollment_id']
dtrain = dtrain.reset_index().merge(aa,on='enrollment_id',how='left').sort('index').drop('index',axis=1)

aa = train.groupby('enrollment_id').std()['time']
aa = pd.DataFrame(aa)
aa['enrollment_id'] = aa.index
aa.columns = ['std_time_problem','enrollment_id']
dtrain = dtrain.reset_index().merge(aa,on='enrollment_id',how='left').sort('index').drop('index',axis=1)

dtrain.index = range(0,len(dtrain))
#loginしてからproblemを見るまでの時間
dtrain['min_time_problem-min_time'] = dtrain['min_time_problem'] - dtrain['min_time']
#最後にproblemをみてからlogoutするまでの時間
dtrain['max_time-max_time_problem'] = dtrain['max_time'] - dtrain['max_time_problem']
#problemを最後に見た時間と最初に見た時間との差
dtrain['max_time_problem-min_time_problem'] = dtrain['max_time_problem'] - dtrain['min_time_problem']
...
dtest.index = range(0,len(dtest))
#loginしてからproblemを見るまでの時間
dtest['min_time_problem-min_time'] = dtest['min_time_problem'] - dtest['min_time']
#最後にproblemをみてからlogoutするまでの時間
dtest['max_time-max_time_problem'] = dtest['max_time'] - dtest['max_time_problem']
#problemを最後に見た時間と最初に見た時間との差
dtest['max_time_problem-min_time_problem'] = dtest['max_time_problem'] - dtest['min_time_problem']
...
Object['start'] = Object['start'].replace('null',np.nan)
Object = Object.dropna()
f_time = lambda x: date(int(x[:4]), int(x[5:7]), int(x[8:10])).toordinal() - 735000
Object['start'] = Object['start'].apply(f_time)

#max
Object_start_max = Object.groupby('course_id').max()['start']
course_index = Object_start_max.index
Object_start_max = pd.DataFrame(Object_start_max.values,columns=['course_start_max'])
Object_start_max.index = course_index
Object_start_max['course_id'] = Object_start_max.index
dtrain = dtrain.reset_index().merge(Object_start_max, on='course_id',how='left').sort('index').drop('index',axis=1)
dtest = dtest.reset_index().merge(Object_start_max, on='course_id',how='left').sort('index').drop('index',axis=1)

dtrain.index = range(0,len(dtrain))
dtest.index = range(0,len(dtest))

dtrain['course_start_max-min'] = dtrain['course_start_max'] - dtrain['min_time']
dtest['course_start_max-min'] = dtest['course_start_max'] - dtest['min_time']

dtrain['course_start_max-max'] = dtrain['course_start_max'] - dtrain['max_time']
dtest['course_start_max-max'] = dtest['course_start_max'] - dtest['max_time']

dtrain['course_end-start'] = dtrain['course_start_max'] - dtrain['start'] 
dtest['course_end-start'] =  dtest['course_start_max'] - dtest['start']

#mean
Object_start_max = Object.groupby('course_id').mean()['start']
course_index = Object_start_max.index
Object_start_max = pd.DataFrame(Object_start_max.values,columns=['course_start_mean'])
Object_start_max.index = course_index
Object_start_max['course_id'] = Object_start_max.index
dtrain = dtrain.reset_index().merge(Object_start_max, on='course_id',how='left').sort('index').drop('index',axis=1)
dtest = dtest.reset_index().merge(Object_start_max, on='course_id',how='left').sort('index').drop('index',axis=1)
...
#std
Object_start_max = Object.groupby('course_id').std()['start']
course_index = Object_start_max.index
Object_start_max = pd.DataFrame(Object_start_max.values,columns=['course_start_std'])
Object_start_max.index = course_index
Object_start_max['course_id'] = Object_start_max.index
dtrain = dtrain.reset_index().merge(Object_start_max, on='course_id',how='left').sort('index').drop('index',axis=1)
dtest = dtest.reset_index().merge(Object_start_max, on='course_id',how='left').sort('index').drop('index',axis=1)
...
#userがproblemをやった時間と'start'時間との差
#print 'leading data'
train = pd.read_csv('/Users/IkkiTanaka/Documents/KDDCup/train/log_train_depth.csv')
test = pd.read_csv('/Users/IkkiTanaka/Documents/KDDCup/test/log_test_depth.csv')
train = train.sort('enrollment_id')
...
train = train.iloc[(train['event']=='problem').values,:]
test = test.iloc[(test['event']=='problem').values,:]
Object_ = Object.iloc[(Object['category']=='sequential').values,:]

#sequantial→vertical
problem_module = []
problem_start = []
for i in xrange(0,len(Object_)):
    x = Object_.iloc[i,:]['children']
    x2 = x.split(' ')
    x2 = x2[0:(len(x2)-1)]
    y = Object_.iloc[i,:]['start']
    for j in xrange(0,len(x2)):
        problem_module.append(x2[j])
        problem_start.append(y)

Object__ = pd.concat([pd.DataFrame(problem_module,columns=['module_id']),pd.DataFrame(problem_start,columns=['start'])],axis=1)

#vertical列
vertical = Object__['module_id'].values
Object = pd.read_csv('/Users/IkkiTanaka/Documents/KDDCup/object.csv')
Object = Object.fillna('0')
Object = Object.drop_duplicates()
Object.index = range(0,len(Object))

Object2 = Object.iloc[(Object['module_id'].isin(vertical)).values,:]
Object2.index = range(0,len(Object2))
Object2['start'] = Object2.reset_index().merge(Object__,on='module_id',how='left')['start_y']


#vertical→problem
problem_module_prob = []
problem_start_prob = []
for i in xrange(0,len(Object2)):
    x = Object2.iloc[i,:]['children']
    x2 = x.split(' ')
    x2 = x2[0:(len(x2)-1)]
    y = Object2.iloc[i,:]['start']
    for j in xrange(0,len(x2)):
        problem_module_prob.append(x2[j])
        problem_start_prob.append(y)

Object2_ = pd.concat([pd.DataFrame(problem_module_prob,columns=['module_id']),pd.DataFrame(problem_start_prob,columns=['start'])],axis=1)


train = train.reset_index().merge(Object2_[['module_id','start']], left_on='object',right_on='module_id',how='inner').sort('index').drop('index',axis=1)
test = test.reset_index().merge(Object2_[['module_id','start']], left_on='object',right_on='module_id',how='inner').sort('index').drop('index',axis=1)
...
#problemの時間 - start時間
train['time-start_prob'] = train['time'] - train['start']
test['time-start_prob'] = test['time'] - test['start']

#train
aa = train.groupby('enrollment_id').mean()['time-start_prob']
aa = pd.DataFrame(aa)
aa['enrollment_id'] = aa.index
aa.columns = ['time-start_prob_mean','enrollment_id']
dtrain = dtrain.reset_index().merge(aa,on='enrollment_id',how='left').sort('index').drop('index',axis=1)

aa = train.groupby('enrollment_id').min()['time-start_prob']
aa = pd.DataFrame(aa)
aa['enrollment_id'] = aa.index
aa.columns = ['time-start_prob_min','enrollment_id']
dtrain = dtrain.reset_index().merge(aa,on='enrollment_id',how='left').sort('index').drop('index',axis=1)

aa = train.groupby('enrollment_id').max()['time-start_prob']
aa = pd.DataFrame(aa)
aa['enrollment_id'] = aa.index
aa.columns = ['time-start_prob_max','enrollment_id']
dtrain = dtrain.reset_index().merge(aa,on='enrollment_id',how='left').sort('index').drop('index',axis=1)

aa = train.groupby('enrollment_id').std()['time-start_prob']
aa = pd.DataFrame(aa)
aa['enrollment_id'] = aa.index
aa.columns = ['time-start_prob_std','enrollment_id']
dtrain = dtrain.reset_index().merge(aa,on='enrollment_id',how='left').sort('index').drop('index',axis=1)

#test
aa = test.groupby('enrollment_id').mean()['time-start_prob']
aa = pd.DataFrame(aa)
aa['enrollment_id'] = aa.index
aa.columns = ['time-start_prob_mean','enrollment_id']
dtest = dtest.reset_index().merge(aa,on='enrollment_id',how='left').sort('index').drop('index',axis=1)

aa = test.groupby('enrollment_id').min()['time-start_prob']
aa = pd.DataFrame(aa)
aa['enrollment_id'] = aa.index
aa.columns = ['time-start_prob_min','enrollment_id']
dtest = dtest.reset_index().merge(aa,on='enrollment_id',how='left').sort('index').drop('index',axis=1)

aa = test.groupby('enrollment_id').max()['time-start_prob']
aa = pd.DataFrame(aa)
aa['enrollment_id'] = aa.index
aa.columns = ['time-start_prob_max','enrollment_id']
dtest = dtest.reset_index().merge(aa,on='enrollment_id',how='left').sort('index').drop('index',axis=1)

aa = test.groupby('enrollment_id').std()['time-start_prob']
aa = pd.DataFrame(aa)
aa['enrollment_id'] = aa.index
aa.columns = ['time-start_prob_std','enrollment_id']
dtest = dtest.reset_index().merge(aa,on='enrollment_id',how='left').sort('index').drop('index',axis=1)
...
#bb = bb.iloc[:,229:]
#cc = bb#[[u'00_h_y', u'01_d_y', u'01_h_y', u'01_m_y', u'02_d_y', u'02_h_y', u'02_m_y', u'03_d_y', u'03_h_y', u'04_d_y', u'04_h_y', u'05_d_y', u'05_h_y', u'05_m_y', u'06_d_y', u'06_h_y', u'06_m_y', u'07_d_y', u'07_h_y', u'07_m_y', u'08_d_y', u'08_h_y', u'08_m_y', u'09_d_y', u'09_h_y', u'0_cg_y', u'0_md_y', u'0_w_y', u'10_d_y', u'10_h_y', u'10_m_y', u'11_d_y', u'11_h_y', u'11_m_y', u'12_d_y', u'12_h_y', u'12_m_y', u'13_d_y', u'13_h_y', u'14_d_y', u'14_h_y', u'15_d_y', u'15_h_y', u'16_d_y', u'16_h_y', u'17_d_y', u'17_h_y', u'18_d_y', u'18_h_y', u'19_d_y', u'19_h_y', u'1_md_y', u'1_w_y', u'2013_y', u'2014_y', u'20_d_y', u'20_h_y', u'21_d_y', u'21_h_y', u'22_d_y', u'22_h_y', u'23_d_y', u'23_h_y', u'24_d_y', u'25_d_y', u'26_d_y', u'27_d_y', u'28_d_y', u'29_d_y', u'2_md_y', u'2_w_y', u'30_d_y', u'31_d_y', u'3_md_y', u'3_w_y', u'4_w_y', u'5_md_y', u'5_w_y', u'6_w_y', u'access_y', u'access_unique_y', u'browser_y', u'browser_acc_y', u'browser_pro_y', u'chapter_cg_y', u'combinedopenended_cg_y', u'course_id_count_y', u'discussion_y', u'discussion_unique_y', u'nagivate_y', u'nagivate_unique_y', u'object_count_all_mean_y', u'object_count_all_std_y', u'object_count_all_sum_y', u'object_count_dtb_mean_y', u'object_count_dtb_std_y', u'object_count_dtb_sum_y', u'page_close_y', u'page_close_unique_y', u'problem_y', u'problem_cg_y', u'problem_unique_y', u'sequential_cg_y', u'server_y', u'server_acc_y', u'server_pro_y', u'sum_count_y', u'username_count_y', u'video_y', u'video_cg_y', u'video_unique_y', u'wiki_y', u'wiki_unique_y','children_0','children_1','children_sum','children_percent','min_time_problem-min_time','max_time-max_time_problem','max_time_problem-min_time_problem','std_time_problem','mean_time_problem','max_time_problem','min_time_problem','min_time_video-min_time','max_time-max_time_video','max_time_video-min_time_video','std_time_video','mean_time_video','max_time_video','min_time_video']]
#cc.columns += '_user_all_new'
#cc.index = range(0,len(cc))
dtest.index = range(0,len(dtest))
...
dtrain_leaf.to_csv('/Users/IkkiTanaka/Documents/ikeda/ikeda_trainfe14.csv',index=False)
dtest_leaf.to_csv('/Users/IkkiTanaka/Documents/ikeda/ikeda_testfe14.csv',index=False)
label.to_csv('/Users/IkkiTanaka/Documents/ikeda/ikeda_labelfe14.csv',index=False)
...
pred_name = []
num_model = 300
for i in xrange(2,num_model,10):
    ...
    num_round = int( round(random.uniform(1280, 1300),0))
...
pred1 = pd.read_csv('/Users/IkkiTanaka/Documents/KDDCup/pred/xgb/pred_all44_0.899005_1461_7.csv',header=None)
pred2 = pd.read_csv('/Users/IkkiTanaka/Documents/KDDCup/pred/xgb/pred_all42_0.899095_1412_7.csv',header=None)
pred3 = pd.read_csv('/Users/IkkiTanaka/Documents/KDDCup/pred/xgb/pred_all42_0.898995_1457_7.csv',header=None)



final_pred = (pred1[1]+pred2[1]+pred3[1])/3.0
final_pred = pd.concat([pred1[0],final_pred],axis=1)
final_pred.to_csv('/Users/IkkiTanaka/Documents/KDDCup/pred/xgb/PRED_ALL8.csv',header=None,index=False)
